...
using ChuanglitouP2P.Model.VeryCodes.NetCreditAssistant;
using ChuanglitouP2P.Model.VeryCodes.NetCreditAssistant.Model;

using ChuanglitouP2P.Common;

namespace ChuanglitouP2P.Bll.VeryCodes.NetCreditAssistant.BLL
{
    ...
    public class InvestCalculator
    {
            ...
            if (inverstParas.CircleType == 1)
            {
                ...
                for (int i = 1; i <= inverstParas.Circle; i++)
                {
                    ...
                    if (i == 1)  //第一期
                    {
                        ...
                        if (dtt > 0)   //当月付息日在投资日之后 如  2016年9月14日投资，当月25日付息
                        {
                            item.interestvalue_date = Startdate_M =startdate= inverstParas.InvestDate;
                            item.Principal = 0M;
                            diffdays = Utils.DateDiff("Day", DateTime.Parse(inverstParas.InvestDate.ToString("yyyy-MM-dd")), DateTime.Parse(CMTH.ToString("yyyy-MM-dd")));
                        ...
                        else //当月付息日在投资日之前 如  2016年9月14日投资，1日付息，得计算到下个月的1号
                        {
                            item.interestvalue_date = Startdate_M = startdate=inverstParas.InvestDate;
                            item.Principal = 0M;
                            DateTime dd = DateTime.Parse(CMTH.AddMonths(1).ToString("yyyy-MM-dd"));
                            diffdays = Utils.DateDiff("Day", DateTime.Parse(inverstParas.InvestDate.ToString("yyyy-MM-dd")), dd);
                            days = Math.Abs(int.Parse(diffdays.ToString()));
                            item.NominalReceiveDate = startdate.AddDays(days);
                            item.TotalDays = days;
                            item.Interest = inverstParas.Amount * ((decimal)(((inverstParas.NominalYearRate / 100.0) * days) / 365.0));
                    ...
                    else if (i == inverstParas.Circle) //最后一期
                    {
                        ...
                        diffdays = Utils.DateDiff("Day", DateTime.Parse(item.interestvalue_date.ToString("yyyy-MM-dd")), CMTH);
                        days = Math.Abs(int.Parse(diffdays.ToString()));
                    ...
                    else
                    {
                        ...
                        diffdays = Utils.DateDiff("Day", DateTime.Parse(inverstParas.InvestDate.ToString("yyyy-MM-dd")), DateTime.Parse(Startdate_M.ToString("yyyy-MM-dd")));
                        days = Math.Abs(int.Parse(diffdays.ToString()));
                        item.NominalReceiveDate = startdate.AddDays(days);
                        item.TotalDays = days;
                        item.Interest = inverstParas.Amount * ((decimal)(((inverstParas.NominalYearRate / 100.0) * days) / 365.0));
        ...
        private static List<InvestmentReceiveRecordInfo> CalculateReceiveRecordWithInterestByMonth1(InvestmentParameters inverstParas)
        {
            ...
            if (inverstParas.CircleType == 1)
            {
                ...
                for (int i = 1; i <= inverstParas.Circle; i++)
                {
                    ...
                    long diffdays = Utils.DateDiff("Day", DateTime.Parse(inverstParas.InvestDate.ToString("yyyy-MM-dd")), DateTime.Parse(CMTH.ToString("yyyy-MM-dd")));
        ...
        private static List<InvestmentReceiveRecordInfo> CalculateReceiveRecordWithLumpSumPrincipalAndInterest(InvestmentParameters inverstParas)
        {
            ...
            long diffdays = Utils.DateDiff("Day", DateTime.Parse(inverstParas.InvestDate.ToString("yyyy-MM-dd")), DateTime.Parse(item.NominalReceiveDate.ToString("yyyy-MM-dd")));
        ...
        public static InvestmentTotalIncome CalculateTotalIncome(InvestmentParameters inverstParas)
        {
            ...
            switch (inverstParas.RepaymentMode)
            {
                ...
                case 3:
                    income = CalculateTotalIncomeWithInterestByMonth(inverstParas);//每月还息，到期还本
                    break;

                case 4:
                    income = CalculateTotalIncomeWithLumpSumPrincipalAndInterest(inverstParas);//一次性还本付息
                    break;
            }
        ...
        /// <param name="value_date">起息日</param>
        /// <param name="repayment_date">还款日</param>
        /// <returns></returns>
