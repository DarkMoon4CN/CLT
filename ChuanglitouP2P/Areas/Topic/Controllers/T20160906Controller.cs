...
{
    public class T20160906Controller : Controller
    {
        // GET: Topic/T20160906
        ...
        {
            rndstr = Utils.RndNumChar(10).ToString();
            ...
            ViewBag.BorrowingTargets = IntialBorrowingTarget();
            ViewBag.tmpData = GetTmpData();
        }
        /// <summary>
        /// 累计为投资人赚取收益
        /// </summary>
        /// <returns></returns>
        private decimal GetTmpData()
        {
        ...
        /// 初始化页面显示的三个借款标数据
        /// </summary>
        private string IntialBorrowingTarget()
        {
            ...
            foreach (M_borrowing_target_ZhuoLu item in listData)
            {
                ...
                targetLifeHtml.AppendFormat("<span>预期年化</span><h3>{0}</h3><small>%</small>", item.annual_interest_rate.ToString("0.00"));
                targetLifeHtml.Append("</div>");
            ...
            return targetLifeHtml.ToString();
        }
        /// <summary>
        /// 根据期限获取借款标数据
        /// </summary>
