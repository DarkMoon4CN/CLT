...
Admin.Loaded = false;
Admin.LogoutUrl = '/Manage/Logout.aspx';
Admin.Init = function () {
    // 其它初始化
...
Admin.SubmitForm = function (verifyFunc) {
    var form = document.forms[0];

    if (J.IsObject(form)) {
        var canSub = true;
        if (J.IsFunction(verifyFunc)) canSub = canSub && verifyFunc() && $(form).validate().form();
        else canSub = canSub && $(form).validate().form();
...
Admin.PostJson = function (url, data, success) {
    J.PostJson(url, data, success, function (a) {
        if (jBox != undefined) {
            RecordError(a.responseText);
            jBox.tip('系统出错！请尝试重新登录', 'loading', {
                timeout: 3000,
                closed: function () {
                    location.href = Admin.LogoutUrl;
                }
...
Admin.GetJson = function (url, data, success) {
    J.GetJson(url, data, success, function (a) {
        if (jBox != undefined) {
            RecordError(a.responseText);
            jBox.tip('系统出错！请尝试重新登录', 'loading', {
                timeout: 3000,
...
var EUrl = "/Handlers/Manage/Error/ErrorInfoHandler.ashx";
function RecordError(error) {
    $.ajax({
        type: "Get",
        url: EUrl,
        data: { method: "RecordError", info: error },
        dataType: "json",
        cache: false
    });
}
...
Index.Open = function (url) {
    if (url != '') {
        ...
        top.$('#content').hide().attr('src', J.FormatString(url));
...
Pager.Output = function (urlFormat, pageSize, pageIndex, pageCount, recordCount) {
    ...
    function _getLink(text, enabled, urlFormat, index) {
        if (enabled == false)
            ...
            return J.FormatString(' <a class="button-white" href="javascript:Pager.Page(' + index + ');"><span>{1}</span></a>', index, text);
    }

    var html = [];
    html.push('<div class="pager-bar">');
    html.push(J.FormatString('<div class="msg">共{0}条记录，当前第{1}/{2}页，每页{3}条记录</div>', recordCount, pageIndex, pageCount, pageSize));
...
Pager.Page = function (index) {
    location.href = J.FormatString(Pager.Data.urlFormat, index);
}
Pager.Jump = function () {
...
Check.CheckAll = function (checkBox, containerId) {
    if (containerId == undefined)//增加事件
        $("input[type=checkbox]").each(function () { this.checked = checkBox.checked; $(this).change(); });
    else {
        $("#" + containerId + " input[type=checkbox]").each(function () { this.checked = checkBox.checked; });
    }
};
Check.GetCheckedIds = function (containerId) {
