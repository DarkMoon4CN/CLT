...
namespace WeiXin.Controllers
{
    public class ThirdpartyController : Controller
    {
        ...
        {                      
            ...
            LogInfo.WriteLog("投标后台主动投标返回报文:" + FastJSON.toJOSN(p));
            string sql = "";
                    ...
                    if (Utils.GeTThirdCache(cachename) == 0)
                    {
                        ...
                        if (p.FreezeTrxId != "")
                        {
                            ...
                            if (dts.Rows.Count > 0)
                            {
                                ...
                                LogInfo.WriteLog("后台投标:id" + id.ToString() + "返回唯一冻结标识:" + p.FreezeTrxId + "事务执行结果:" + d.ToString());
                                if (d > 0)
                                {
                                    ...
                                    if (dt.Rows.Count > 0)
                                    {
                                        ...
                                        #region 下线 投资六月专享标一元抢Iphone 2016年9月11日 至2016年9月30日
                                        if (false)
                                        {
                                            ...
                                            if (nowdate > startdate && nowdate < enddate)
                                            {
                                                ...
                                                log += "; 标的期限：" + dt.Rows[0]["life_of_loan"].ToString();
                                                if (dt.Rows[0]["unit_day"].ToString() == "1" && dt.Rows[0]["life_of_loan"].ToString() == "6")//是否六月标
                                                {
                                                    log += "; 用户注册时间：" + mmt.Registration_time.ToString();
        ...
        public ActionResult Bgthirdpartybindbank()
        {
            ...
            LogInfo.WriteLog("签名返回结果:" + ret.ToString());

            LogInfo.WriteLog("绑卡报文:" + FastJSON.toJOSN(m));
            if (ret == 0)
            {
