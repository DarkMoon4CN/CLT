...
namespace ChuanglitouP2P.DAL
{
	...
	public partial class D_province
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "province"); 
		}
		...
		public int Add(M_province model)
		{
			...
			strSql.Append("provinceID,province)");
		...
		public M_province GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,provinceID,province from province ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			M_province model=new M_province();
		...
		/// 得到一个对象实体
		/// </summary>
		public M_province DataRowToModel(DataRow row)
		{
			M_province model=new M_province();
			if (row != null)
			{
