GO
/****** Object:  StoredProcedure [dbo].[prSubmitTender]    Script Date: 2016/12/9 11:11:32 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[prSubmitTender]
	@usrId [int],
	@targetId [int],
...
BEGIN
	...
	DECLARE @life_of_loan int;--周期数值
	DECLARE @unit_day int;--周期类型
	DECLARE @UseLifeLoan nvarchar(50);--代金券使用期限限定(A-B)
	SET @aor=0;
	SET @JiaxiNum=0;
	SET @BonusAmt=0;
	SET @RewTypeID=0;
...
BEGIN TRAN submitTender;    --开始事务
	...
	SELECT @targetBalance = ( borrowing_balance-fundraising_amount),@life_of_loan=life_of_loan,@unit_day=unit_day FROM dbo.hx_borrowing_target WHERE targetid=@targetId;
		...
		BEGIN--判断用户余额
			...
			BEGIN
				...
				BEGIN
					...
					begin
						SET @RewTypeID=0;
						SELECT @lmt=uselower,@RewTypeID=RewTypeID,@Amt=Amt,@UseLifeLoan=UseLifeLoan FROM dbo.hx_useract WHERE registerid=@usrId AND useract=@bonusId AND AmtEndtime>=convert(nvarchar(4),year(getdate()))+'-'+convert(nvarchar(2),MONTH(getdate()))+'-'+convert(nvarchar(2),DAY(GETDATE()));
						PRINT @lmt;
						IF(@lmt>@investAmount or @RewTypeID=0 or [dbo].[IsBonusUseable]([dbo].[GetTartgetLoanMonth](@life_of_loan,@unit_day),@UseLifeLoan)=0)
						BEGIN
							PRINT '个别优惠券或者红包不可用';
							ROLLBACK TRAN;
							RETURN -500;
				...
				BEGIN
					...
					BEGIN
						...
						BEGIN
							...
							BEGIN
							--锁定优惠券
								update hx_UserAct set UseState=3,AmtProid=@recordId where UserAct=@bonusId and registerid=@usrId and UseState=0 
             				END
							ELSE
							BEGIN
							 	ROLLBACK TRAN;
