		...
		BEGIN--判断用户余额
			...
			BEGIN
				...
				BEGIN
					...
					begin
						SET @RewTypeID=0;
						SELECT @lmt=uselower,@RewTypeID=RewTypeID,@Amt=Amt,@UseLifeLoan=UseLifeLoan FROM dbo.hx_useract WHERE registerid=@usrId AND useract=@bonusId;
						PRINT @lmt;
						IF(@lmt>@investAmount or [dbo].[IsBonusUseable]([dbo].[GetTartgetLoanMonth](@life_of_loan,@unit_day),@UseLifeLoan)=0)
						BEGIN
							PRINT ''个别优惠券或者红包不可用'';
							ROLLBACK TRAN;
							RETURN -500;
